...
TempVector ASL::$(Vector v, Vector u)	// overloaded version $
{
	TempVector w;
	unsigned int c;
	anticast p;
...
@// vectors v and u are assumed to be of the same size.
TempVector ASL::$(TempVector v, Vector u)	// overloaded version $
{
	unsigned int c;
	anticast p;
...
@// vectors v and u are assumed to be of the same size.
TempVector ASL::$(Vector u, TempVector v)	// overloaded version $
{
	unsigned int c;
	anticast p;
...
@// vectors v and u are assumed to be of the same size.
TempVector ASL::$(TempVector v, TempVector u)	// overloaded version $
{
	unsigned int c;
	anticast p,q;
...
@TempVector ASL::$(double x, Vector v)	// overloaded version $
{
	unsigned int c;
	TempVector u;
	anticast p;
...
@TempVector ASL::$(Vector v, double x)	// overloaded version $
{
	unsigned int c;
	TempVector u;
	anticast p;
...
@TempVector ASL::$(double x, TempVector v)	// overloaded version $
...
@TempVector ASL::$(TempVector v, double x)	// overloaded version $
{	/* we will recycle the memory used by v. */
	unsigned int c;
	anticast p;
...
TempMatrix ASL::$(Matrix A, Matrix B)	// overloaded version $
{
...
@// matrices A and B are assumed to be of the same size.
TempMatrix ASL::$(TempMatrix A, Matrix B)	// overloaded version $
...
@// matrices A and B are assumed to be of the same size.
TempMatrix ASL::$(Matrix B, TempMatrix A)	// overloaded version $
{	/* we will recycle the memory used by A. */
	register unsigned int c;
	register anticast p;
...
@// matrices A and B are assumed to be of the same size.
TempMatrix ASL::$(TempMatrix A, TempMatrix B)	// overloaded version $
{	/* we will recycle the memory used by A. */
	register unsigned int c;
...
@TempMatrix ASL::$(double x, Matrix A)	// overloaded version $
{
	register unsigned int c;
	register anticast p;
	TempMatrix B;
...
@TempMatrix ASL::$(Matrix A, double x)	// overloaded version $
{
	register unsigned int c;
	TempMatrix B;
	register anticast p;
...
@TempMatrix ASL::$(double x, TempMatrix A)	// overloaded version $
{	/* we will recycle the memory used by A. */
	register unsigned int c;
	register anticast p;
...
@TempMatrix ASL::$(TempMatrix A, double x)	// overloaded version $
{	/* we will recycle the memory used by A. */
	register unsigned int c;
	register anticast p;
